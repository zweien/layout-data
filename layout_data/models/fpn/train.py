...
if __name__ == "__main__":
    ...
    dataset = GeneralDataset(
        trans_separate, resize_shape=(200, 200)
    )  # 生成数据集，trans_separate是什么参数
    dataset_test = TestDataset(trans_separate, resize_shape=(200, 200))

    train_loader = DataLoader(dataset, batch_size=batch_size, shuffle=True)
    valid_loader = DataLoader(
        dataset_test, batch_size=16, shuffle=False, drop_last=True
    )
    ...
    scheduler = WarmupMultiStepLR(
        optimizer, milestones=[], warmup_iters=len(train_loader)
    )
    ...
    for epoch in range(max_epochs):
        ...
        for it, images in enumerate(train_loader):
            ...
            print(
                "\tEpoch[{}/{}] Iters[{}/{}] Loss: {:.3f}".format(
                    epoch + 1, max_epochs, it, len(train_loader), loss.item() * 1e5
                ),
            )
        ...
        if epoch % model_valid_interval == 0:
            ...
            with torch.no_grad():
                mae, accuracy = validate(
                    model, valid_loader, max_iter=-1, flag_detail=False
                )
                print(
                    "1-MAE:", round(mae * 100, 4), "Accuracy:", round(accuracy * 100, 4)
                )
            model.train()
